...
--- QUE 6) Use a recursive CTE to generate a hierarchail list of categories and their sub-categories from category table in sakila database ?
	...
	-- CTE with Joins
--- QUE 7) Create a CTE that combines information from the film and language tables to display film_tile , language name and rental rate?
...
--- QUE 12) Use a CTE to pivot the data from the payment to display the total payments  made by each customer in separate columns for different payment methods
