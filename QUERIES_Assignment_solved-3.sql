...
inventory i ON f.film_id = i.film_id
 ...
 where c.customer_id in (SELECT r.customer_id from rental r join customer c ON r.customer_id = c.customer_id where c.email like '%.org');
...
SELECT 
    c.customer_id, c.first_name, c.last_name , count(r.customer_id) As total_rental
FROM
    customer c
        INNER JOIN
    rental r ON c.customer_id = r.customer_id
